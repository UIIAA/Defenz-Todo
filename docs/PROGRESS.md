...
## Current focus
**`feature-time-entries` (Desenho B) IMPLEMENTADA — código + gate verde, NÃO deployada nem commitada-em-prod.** Também pendente: deploy de Fases B+D (commit `1fb3fcf`).

- **feature-time-entries DONE (código):** diário de horas delta-on-save + campo `Demanda.client` + aba `/dashboard/demandas/horas`. 31 testes novos (**523 total**), `tsc`+`build` verdes. Revisão adversarial multi-agente (2 rodadas) → achados corrigidos. **Pendências de deploy (requerem autorização p/ tocar o Neon):** (1) `npx prisma db push` (aditivo: coluna `client` + tabela `time_entries` — bloqueado pelo classifier nesta sessão), (2) `npx tsx scripts/backfill-time-entries.ts` (seed auto-corretivo/idempotente — pode rodar a qualquer momento; insere baseline = total − Σ lançamentos), (3) validar a aba Horas autenticado. **Sem isso, deployar quebra `/api/time-entries` e o save com `client` (coluna inexistente).** Ver [feature-time-entries.md](features/feature-time-entries.md).
...
0. **DEPLOY de `feature-time-entries`** (requer autorização p/ Neon): (a) `npx prisma db push` — aditivo, cria coluna `demandas.client` + tabela `time_entries` (ADR-008, single Neon dev==prod); (b) `npx tsx scripts/backfill-time-entries.ts` — seed baseline auto-corretivo/idempotente; (c) validar a aba `/dashboard/demandas/horas` autenticado (criar/editar horas → ver lançamento; subtarefa idem; agrupar por cliente/responsável/equipe/área/card; filtro de período). **Só então** `git push` (Vercel auto-deploy). ⚠️ db push DEVE preceder o deploy do código (senão `/api/time-entries` e o save com `client` quebram).
1. **Deploy de B+D** (decisão do usuário): `git push` na main → Vercel auto-deploya. Backward-compatible e validado (build verde). Sem migration nova (schema da Fase A já aplicado). Pós-deploy: testar UI autenticada em prod (multi-select Empresas) + MCP contra prod.
...
- 2026-06-08 **`feature-time-entries` (Desenho B) IMPLEMENTADA** — diário de horas delta-on-save + campo `Demanda.client` + aba `/dashboard/demandas/horas`. Schema (`Demanda.client`, modelo `TimeEntry`), helpers puros (`src/lib/time-entries.ts`) + gravação (`src/lib/time-entries-server.ts` `logTimeDelta`), hooks de delta em `PUT /api/demandas` + 3 rotas de subtarefa (POST/PUT/DELETE), `GET /api/time-entries` (admin/gerência, escopo por conjunto + filtros + TZ SP + cap 5000), modal ganha campo Cliente (board/horas inalterados), nav admin/gerência, seed `scripts/backfill-time-entries.ts` (baseline auto-corretivo). **31 testes novos (523 total), tsc+build verdes.** Revisão adversarial multi-agente (6 dims → verify, 2 rodadas) → 1 alto (order-independence do seed) + 2 médios (TZ SP no filtro, teste de persistência do client) + baixos, **todos corrigidos**. **NÃO deployado / NÃO commitado em prod** — `db push` bloqueado pelo classifier (toca Neon). Specs: feature-time-entries.
